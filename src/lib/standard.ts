...
export const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    items: [
      { key: 'colour_match', group: 'Fn', label: { en: 'Colour match wheel vs cap', zh: '轮毂与盖颜色一致' },
        standard: { en: 'Same as approved sample', zh: '与认可样品一致' } },
      { key: 'cap_fitment', group: 'Fn', label: { en: 'Cap fitment', zh: '盖子配合' },
        standard: { en: 'Cap fits tightly on wheel', zh: '盖子须紧密配合轮毂' } },
      { key: 'logo', group: 'Fn', label: { en: 'Logo', zh: '标志' },
        standard: { en: 'Same as approved sample', zh: '与认可样品一致' } },
      { key: 'bolt_cone_paint', group: 'Fn', label: { en: 'Bolt hole / cone free of paint', zh: '螺栓孔/锥座无涂料' },
        standard: { en: 'Free of paint (Standard p.5)', zh: '无涂料（标准第5页）' } },
      { key: 'rear_bore_paint', group: 'Fn', label: { en: 'Rear centre bore + mounting face paint-free', zh: '背面中心孔/安装面无涂料' },
        standard: { en: 'Free of paint (Standard p.5)', zh: '无涂料（标准第5页）' } },
      { key: 'hat_marks', group: 'Fn', label: { en: 'No hat marks', zh: '无压痕' },
        standard: { en: 'Wheel face free of visible hat marks (Standard p.5)', zh: '正面无可见压痕（标准第5页）' } },
      { key: 'orange_peel', group: 'Fn', label: { en: 'Smooth surface, no orange peel', zh: '表面光滑无橘皮' },
        standard: { en: 'Per SOP', zh: '按SOP' } },
      { key: 'cap_finish', group: 'Fn', label: { en: 'Cap surface finish', zh: '盖子表面处理' },
        standard: { en: 'Matches approved wheel finish sample (Standard p.5)', zh: '与认可轮毂样品一致（标准第5页）' } },
      { key: 'tpms', group: 'Fn', label: { en: 'TPMS (if fitted)', zh: 'TPMS（如有）' },
        standard: { en: 'Same as sample (company spec — confirm w/ SKU)', zh: '与样品一致（公司规格）' } },
      { key: 'coating_total', group: 'Fn', label: { en: 'Total coating thickness', zh: '涂层总厚度' },
        standard: { en: 'Min. between 120–130 µm (Standard p.5)', zh: '最小120至130µm（标准第5页）' } },
      { key: 'coating_machined', group: 'Fn', label: { en: 'Machined-area coating', zh: '加工面涂层' },
...
export const MEAS_COLS: MeasCol[] = [
  { key: 'counter_bore', label: { en: 'Counter bore', zh: '埋头孔' },
    nominal: s => s.counter_bore_mm, tol: { en: '±0.50', zh: '±0.50' },
    check: (v, s) => Math.abs(v - s.counter_bore_mm) <= 0.5 },
  { key: 'lug_hole', label: { en: 'Lug hole', zh: '螺栓孔' },
    nominal: s => s.lug_hole_mm, tol: { en: '±0.25', zh: '±0.25' },
    check: (v, s) => Math.abs(v - s.lug_hole_mm) <= 0.25 },
  { key: 'seat_thick', label: { en: 'Seat thickness', zh: '座厚' },
    nominal: s => s.seat_thickness_mm, tol: { en: '±0.50', zh: '±0.50' },
    check: (v, s) => Math.abs(v - s.seat_thickness_mm) <= 0.5 },
  { key: 'offset', label: { en: 'Offset ET', zh: '偏距' },
    nominal: s => s.offset_mm, tol: { en: '±1.00', zh: '±1.00' },
    check: (v, s) => Math.abs(v - s.offset_mm) <= 1.0 },
  { key: 'cb', label: { en: 'CB', zh: '中心孔' },
    nominal: s => s.cb_mm, tol: { en: '+0/+0.10', zh: '+0/+0.10' },
    check: (v, s) => v - s.cb_mm >= 0 && v - s.cb_mm <= 0.10 },
  { key: 'coating_total', label: { en: 'Coating total (µm)', zh: '涂层总厚(µm)' },
    nominal: () => 125, tol: { en: '120–130', zh: '120–130' },
    check: v => v >= 120 && v <= 130 },
  { key: 'coating_machined', label: { en: 'Machined coat (µm)', zh: '加工面涂层(µm)' },
    nominal: () => 80, tol: { en: '≥80', zh: '≥80' },
    check: v => v >= 80 },
  { key: 'radial_top', label: { en: 'Radial top', zh: '径向上' },
    nominal: s => runoutLimits(s.diameter_in).radial, tol: { en: 'max', zh: '最大' },
  ...
  { key: 'axial_top', label: { en: 'Axial top', zh: '轴向上' },
    nominal: s => runoutLimits(s.diameter_in).axial, tol: { en: 'max', zh: '最大' },
