...
export function DefectModal({ inspectionId, itemKey, itemLabel, pieceNo, tab, onDone, onClose }: BaseProps) {
  ...
  const save = async () => {
    ...
    const { data: defect } = await supabase.from('defects').insert({
      ...
      comment, is_extra_piece: false,
    }).select('id').single()
    if (defect && photoPath) {
      await supabase.from('photos').insert({
        inspection_id: inspectionId, defect_id: defect.id,
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="row" style={{ marginTop: 14 }}>
          <button className="btn ok" style={{ flex: 1 }} disabled={!photoPath || saving} onClick={save}>
            {saving ? '…' : 'Save / 保存'}
          </button>
          <button className="btn ghost" onClick={onClose}>Cancel</button>
        </div>
