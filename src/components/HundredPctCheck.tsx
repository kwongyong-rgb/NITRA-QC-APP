...
export default function HundredPctCheck({ inspectionId, lotSize, triggeredItems, results, onSave, editable }: Props) {
  // i18n available if needed
  const [activeItem, setActiveItem] = useState(triggeredItems[0]?.key || '')
  const [modal, setModal] = useState<ModalState>(null)

  const item = triggeredItems.find(i => i.key === activeItem)
  if (!item) return null

  const checked = Array.from({ length: lotSize }, (_, i) => i + 1)
    .filter(n => results[`${activeItem}:${n}`] !== undefined).length
  const fails = Array.from({ length: lotSize }, (_, i) => i + 1)
    .filter(n => results[`${activeItem}:${n}`] === 'F').length

  const setResult = (pieceNo: number, val: PFNA) => {
    onSave(activeItem, pieceNo, val)
  }

  // Group pieces into rows of 10 for compact display
  const rows: number[][] = []
  ...
  return (
    <div className="card">
      ...
      {/* Item selector if multiple triggered */}
      {triggeredItems.length > 1 && (
        <div className="tabs" style={{ position: 'static', marginBottom: 12 }}>
          {triggeredItems.map(it => (
            <button key={it.key} className={activeItem === it.key ? 'on' : ''} onClick={() => setActiveItem(it.key)}>
              {it.label}
            </button>
          ))}
        </div>
      ...
      <div className="muted" style={{ marginBottom: 10, fontSize: 13 }}>
        Checking for: <b>{item.label}</b> · Tap each piece to record P or F
      </div>
      ...
      {rows.map((row, ri) => (
        <div key={ri} style={{ display: 'flex', gap: 4, marginBottom: 4, flexWrap: 'wrap' }}>
          {row.map(n => {
            const val = results[`${activeItem}:${n}`]
            return (
              <div key={n} style={{ position: 'relative' }}>
                <div style={{
                  ...
                  {/* Piece number */}
                  <div style={{ textAlign: 'center', fontSize: 11, fontWeight: 700, padding: '3px 0', borderBottom: '1px solid var(--line)', background: 'rgba(0,0,0,.04)' }}>
                    {n}
                  </div>
                  {/* P / F buttons */}
                  <div style={{ display: 'flex' }}>
                    <button
                      disabled={!editable}
                      style={{
                        flex: 1, border: 'none', borderRight: '1px solid var(--line)', minHeight: 36,
                        background: val === 'P' ? 'var(--pass)' : 'transparent',
                        color: val === 'P' ? '#fff' : 'var(--pass)', fontWeight: 700, fontSize: 13, cursor: 'pointer'
                      }}
                      onClick={() => {
                        setResult(n, val === 'P' ? undefined : 'P')
                        if (val !== 'P') setModal({ type: 'pass', itemKey: activeItem, itemLabel: item.label, pieceNo: n })
                      }}>P</button>
                    <button
                      disabled={!editable}
                      style={{
                        flex: 1, border: 'none', minHeight: 36,
                        background: val === 'F' ? 'var(--fail)' : 'transparent',
                        color: val === 'F' ? '#fff' : 'var(--fail)', fontWeight: 700, fontSize: 13, cursor: 'pointer'
                      }}
                      onClick={() => {
                        setResult(n, val === 'F' ? undefined : 'F')
                        if (val !== 'F') setModal({ type: 'fail', itemKey: activeItem, itemLabel: item.label, pieceNo: n })
                      }}>F</button>
                  </div>
                </div>
              </div>
      ...
      {modal?.type === 'fail' && (
        <DefectModal
          ...
          pieceNo={modal.pieceNo} tab="form"
