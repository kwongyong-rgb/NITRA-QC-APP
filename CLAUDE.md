...
**Suggested next batch (as of v90):** the offline READ side is now complete and device-verified. Remaining, in order:
1. **Offline container-loading creation** — the last Stage 2 gap. v86 did wheel inspections only; `ContainerLoading.tsx` + `PoHub.addContainer` need the same client-UUID + pending-store + sync treatment, and v90's list-merge pattern extends to it. Smallest remaining piece.
2. **Stage 3 — offline photos/videos.** A QC inspection is photo-backed by definition, so offline capture without photos is still half a form. Photos are additive separate rows, which puts them on the no-collision side of the Stage 4 merge design.
3. **Stage 4 — conflict/merge.** Kwong has repeatedly flagged the two-user shared-SKU clash (one online marks All Pass, one offline marks All Fail) as the scenario he most wants handled.
