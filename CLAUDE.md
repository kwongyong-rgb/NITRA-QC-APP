...
- **`isOffline()` (v87, `connectivity.ts`) is for BLOCKING WRITES ONLY — never for deciding what to render.** It uses `navigator.onLine === false`. **v88 PROVED THIS THE HARD WAY ON A REAL DEVICE:** in an iOS **standalone PWA in airplane mode, `navigator.onLine` still reports `true`**, so `isOffline()` returns false while genuinely offline. v87 used it for the PO-list empty state (violating this very rule, written one batch earlier) and the screen showed the ONLINE message while the pill correctly said Offline. Every user-facing connectivity decision must use the ping-confirmed **`useOnline()`** hook — the same source as the header pill, which was right all along. `isOffline()` now survives in ONE place only: the lazy PO-create guard in `poStatus.getOrCreatePoId()` / `PoInfo.load`, which is non-component code with no hook available; a false "online" there is harmless because the insert simply fails with a network error and creates nothing.
