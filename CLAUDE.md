...
5. **`offlineMedia.ts`** (v91, Stage 3) — offline **photos & videos**. Own IndexedDB DB (`nitra-qc-media`) with two stores: `blobs` (the captured file, keyed by its FUTURE storage path) and `rows` (the queued `photos` table row). Same trick as v86: the storage path is client-minted BEFORE upload, so an offline capture is stored under the exact path it will occupy in the bucket and sync just uploads to it — nothing to reconcile. Key exports: `saveLocalMedia`, `savePendingPhotoRow`, `mediaUrlFor` (local blob first, else signed URL), `getPendingPhotosFor`, `pendingMediaStats`, `syncPendingMedia`, `currentUserId` (reads the persisted session — works offline, avoids threading a prop through every photo modal).
   ...
   - **v104 CHANGED THE ONLINE PATH ON PURPOSE (approved as option "B"):** inspection photos now ALWAYS go through the local queue + background upload, online too — the inline `photos` insert was replaced. Capture → `compressImage` (~1.2 MB, ~2000px, videos untouched, fail-safe to original) → `saveLocalMedia` → queue the row → kick `syncPendingMedia` → user saves and moves on INSTANTLY. The 📷 +1 shows from the queue immediately (fixed "save before upload finishes → +1 doesn't appear"); the background upload then removes the local copy. Gated by the `deferUpload` prop on `MediaCapture`: DefectModal / PassPhotoModal / appendix pass it; CONTAINER photos do NOT (they still upload inline, online-only) but do get compression. The capture onChange handlers stay synchronous fire-and-forget (v98) — only what happens AFTER capture changed. So "online path is frozen" now means: capture handler shape is frozen; the post-capture queue flow is the intended design.
   - Local blob capture (for container / fallback) is only ever used after a real network failure or offline.
   - **iOS EMPTY-BLOB SAGA (v95–v98) — read ALL of this before touching MediaCapture; it cost four test cycles:**
     1. **The real bug (fixed in v96):** storing the RAW File from `<input capture>` in IndexedDB. That File merely REFERENCES an iOS camera temp file; read back in a later session the temp is gone, so the blob uploads empty → `upload: No content provided` → the ⏳ counter sticks. `saveLocalMedia` must materialize the bytes via `await file.arrayBuffer()` into an in-memory Blob at save time. **Do not regress `saveLocalMedia` to storing the raw File.**
     2. **Self-heal (v97):** a stale File reference reports a NON-ZERO `.size` with its content gone, so a size check can't catch pre-v96 queue entries. `syncPendingMedia` drops a queued item when the UPLOAD fails with "No content provided"/empty-body (bytes unrecoverable — the inspector re-takes the shot); other upload errors stay queued and retry.
     3. **v97's OTHER change was a MISDIAGNOSIS that broke ONLINE capture (reverted in v98):** v97 also made the `onChange` handlers async (`await upload(...)` before clearing `input.value`) on the theory that clearing the input releases the temp file before the read. That theory is DISPROVEN by the evidence: online uploads have ALWAYS read the File after the input was cleared (fire-and-forget + immediate `value=''`) and worked from day one through v96. The async form broke online photo capture on iOS (photos silently not recorded). **The onChange handlers must stay synchronous fire-and-forget — never re-introduce an await there.** Exact failure correlation: online capture worked v91→v96, broke at v97, v97's only capture-path change was that handler.
     4. **Counter staleness (fixed v98):** `App.tsx`'s reconnect effect refreshed the ⏳ tally only when `syncPendingMedia` returned uploads > 0 — but the discard path clears queue items without counting as uploads, and the 15s poll's guarded sync can collide with the reconnect sync and read mid-run stats. Refresh the tally UNCONDITIONALLY after any sync.
...
- **"All my photos disappeared" was a DISPLAY bug, never data loss (v100) — check the DB before panicking.** Diagnostic SQL against production showed every photo present, so the hunt went straight to the render path instead of the write path. Cause: the offline load path deliberately does `setServerPhotos([])` (server images need the network), and `load()`'s deps don't include `online`, so it never re-ran on reconnect — the Photos tab stayed blank until a manual navigate-away-and-back. Fixed with a reconnect effect on `Inspection` that pushes offline edits THEN calls `load()`. **Any screen that empties state on the offline path must re-load when connectivity returns.** When a report sounds like data loss, run a read-only SELECT against `photos`/`inspections` first — it costs one round trip and eliminates guessing.
- **A deleted inspection must take its QUEUED offline photos with it (v102).** Offline photos live in the `nitra-qc-media` queue keyed by their future storage path, with the inspection_id baked in. Deleting the inspection orphans them — the `photos` insert then fails forever (`new row violates row-level security policy`, no parent row) and the ⏳ counter sticks on an unrelated screen. Two layers: `discardQueuedMediaForInspection(id)` is called from the delete paths (`PoHub.delInsp`/`delPO`), AND `syncPendingMedia` runs an orphan-cleanup pass (parent gone from BOTH server and pending store → discard) as a backstop for any delete path — guarded so a flaky network never wrongly discards a valid queued photo. Any NEW inspection-delete path must call `discardQueuedMediaForInspection`.
- **NEVER make a Supabase call you know will fail — offline they HANG, they don't fail fast (v100).** An offline Supabase call waits for the OS connection timeout (~a minute for the first; quicker afterwards once iOS marks the host unreachable). Symptom: the first offline photo save took ~1 min, the second was fast. Because `Inspection.load()` runs after EVERY action (mark a result, save a photo) and began with two doomed reads, the whole screen felt frozen offline. Every such path is now guarded with `isOffline()`: `DefectModal.save`, `PassPhotoModal.save`, `ensureDefect`, `removeDefect`, `saveFd`, `setResult`'s photo-verdict sync, and `load()`'s inspection + SKU reads. **When adding any new Supabase call on the inspection path, guard it with `isOffline()` and fall through to the offline path directly.**
